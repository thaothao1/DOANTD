...
# @app.get("/districts")
# def get_list_districts(skip: int  = 0, limit: int = 100 , db : Session = Depends(get_db)):
#     data = cruds.districts.getById(db,skip,limit)
#     if data == None:
#         return HTTPException(status_code=400 , detail="false")
#     return custom_reponse(http_status=200 , data= data)

# @app.post("/districts")
# def createDistricts( body : DistrictCreate , db: Session = Depends(get_db)):
