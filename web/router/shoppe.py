...
def info_items(data ,key , db):
    ...
    for item in items:
        k = item['item_basic']
        name =  k['name']
        image = "https://cf.shopee.vn/file/" + k['image']
        price_min =  int(k['price_min']) / 100000
        price_max = int(k['price_max']) / 100000
        rating = k['item_rating']['rating_star']
        link ="https://shopee.vn/product/{}/{}".format(k['shopid'], k['itemid'])
        listDict = Product(
            name = str(name),
            link = str(link),
            image = str(image),
